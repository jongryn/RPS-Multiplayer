...
//            Jon (9/27/17) - Fixed Firebase, JS, bugs, and now RPS is working
*/
...
database.ref("/players/").on("value", function(snapshot) {
	...
	if (snapshot.child("player1").exists()) {
		...
		$("#roundOutcome").html("Rock-Paper-Shotgun");
		$("#waitingNotice").html("");
...
function rpsCompare() {
	if (player1.choice === "Rock") {
		...
		if (player2.choice === "Rock") {
			...
			// Player1 wins
			console.log("shotgun win");

			database.ref().child("/outcome/").set("Shotgun win!");
			database.ref().child("/players/player1/win").set(player1.win + 1);
			database.ref().child("/players/player2/loss").set(player2.loss + 1);
		} else {
			// Tie
